...
class SpeciesByAvgWeight(APIView):
    def get(self, request):
        queryset = Specie.objects.annotate(avg_weight=Avg('content__kilograms')).order_by('-avg_weight')
...
class AssignmentList(APIView):
    ...
    def post(self, request, pk, format=None):
        try:
            ...
            assignment.starting_date = request.data.get("starting_date")
            assignment.ending_date = request.data.get("ending_date")
            assignment.save()
            serializer = AssignmentSerializer2(assignment)
